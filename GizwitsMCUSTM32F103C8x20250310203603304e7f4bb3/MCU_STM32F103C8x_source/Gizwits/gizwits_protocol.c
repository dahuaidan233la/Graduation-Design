...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLinht_value != cur->valueLinht_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLinht_value Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagLinht_value = 1;
            ret = 1;
        }
    }
    if(last->valueMQ2_value != cur->valueMQ2_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagMQ2_value = 1;
            ret = 1;
        }
    }
    if(last->valueMQ135_value != cur->valueMQ135_value)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagMQ135_value = 1;
            ret = 1;
        }
    }
    if(last->valueID != cur->valueID)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueID Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagID = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagLinht_value)
    {
        devStatusTmp.valueLinht_value = exchangeBytes(gizY2X(Linht_value_RATIO,  Linht_value_ADDITION, dataPoints->valueLinht_value)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueLinht_value,Linht_value_LEN);
        byteFieldOffset += Linht_value_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagMQ2_value)
    {
        devStatusTmp.valueMQ2_value = exchangeBytes(gizY2X(MQ2_value_RATIO,  MQ2_value_ADDITION, dataPoints->valueMQ2_value)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueMQ2_value,MQ2_value_LEN);
        byteFieldOffset += MQ2_value_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagMQ135_value)
    {
        devStatusTmp.valueMQ135_value = exchangeBytes(gizY2X(MQ135_value_RATIO,  MQ135_value_ADDITION, dataPoints->valueMQ135_value)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueMQ135_value,MQ135_value_LEN);
        byteFieldOffset += MQ135_value_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagID)
    {
        devStatusTmp.valueID = exchangeBytes(gizY2X(ID_RATIO,  ID_ADDITION, dataPoints->valueID)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueID,ID_LEN);
        byteFieldOffset += ID_LEN;
    }
